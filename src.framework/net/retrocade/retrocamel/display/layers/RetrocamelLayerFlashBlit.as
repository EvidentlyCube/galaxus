...
package net.retrocade.retrocamel.display.layers{
    ...
    public class RetrocamelLayerFlashBlit extends RetrocamelLayerFlash{
        ...
        public function drawAdvanced(data:*, x:Number, y:Number, rotation:Number = 0, scaleX:Number = 1, scaleY:Number = 1, blend:String = null, smoothing:Boolean = false, alpha:Number = 1):void{
            if (rotation == 0 && scaleX == 1 && scaleY == 1 && blend == null && (data is Bitmap || data is BitmapData) && alpha == 1){
                if (data is Bitmap)
                    draw(Bitmap(data).bitmapData, x, y);
                else
                    draw(data, x, y);
